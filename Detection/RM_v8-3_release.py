...
## Definitions
# Define directories
Extent_dir = "DEM/FullScale/InputData/DEM/"
Lakes_in_dir = "DEM/GlobalLakes/"
Løsmasse_in_dir = "DEM/løsmasse/Nationwide/"
Out_dir = "DEM/FullScale/InputData/"
...
for filename in os.listdir(Extent_dir):
    if filename.endswith(".tif"):
        ...
        # Fill Holes in Raster gdal (defunct)
        ...
        # Fill Holes in Raster rasterio
        demr = ExtentRaster
        with rasterio.open(demr) as src:
            profile = src.profile
            arrdem = src.read(1)
            arr_filleddem = fillnodata(arrdem, mask=src.read_masks(1), max_search_distance=30, smoothing_iterations=0)
        with rasterio.open(demr, 'w', **profile) as dest:
            dest.write_band(1, arr_filleddem)
        ...
        OutVector_Lmass = Out_dir + "Løsmasse/Løsmasse_" + index_fn + ".shp"
        LmassIn = OutVector_Lmass
        LmassOut = LmassIn[:-3] + 'tif'

        # Lakes
        LakesPoly = Lakes_in_dir + "Innsjo_Innsjo.shp"
        OutVector_Lakes = Out_dir + "Lakes/Lakes_" + index_fn + ".shp"
...
Lakes_dir = 'DEM/FullScale/InputData/Lakes/'
Lmass_dir = 'DEM/FullScale/InputData/Løsmasse/'

# Path DOES NOT look into subdirs on its own volition.
for item in Path(Lmass_dir).glob("*.shp"):
    item.unlink()
for item in Path(Lmass_dir).glob("*.shx"):
    item.unlink()
for item in Path(Lmass_dir).glob("*.prj"):
    item.unlink()
for item in Path(Lmass_dir).glob("*.dbf"):
    item.unlink()
for item in Path(Lakes_dir).glob("*.shp"):
    item.unlink()
for item in Path(Lakes_dir).glob("*.shx"):
    item.unlink()
for item in Path(Lakes_dir).glob("*.prj"):
    item.unlink()
for item in Path(Lakes_dir).glob("*.dbf"):
    item.unlink()
...
DEM_dir = Extent_dir
TWI_dir = 'DEM/FullScale/InputData/TWI/'
...
for filename in os.listdir(DEM_dir):
    if filename.endswith(".tif"):
        ...
        with rasterio.open('DEM/FullScale/OutputData/33_' + index_fn + "_Cluster.tif",
                           'w',
                           driver='GTiff',
                           height=sel.shape[0],
                           width=sel.shape[1],
